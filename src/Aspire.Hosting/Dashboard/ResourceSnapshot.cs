...
internal abstract class ResourceSnapshot
{
    ...
    public required ImmutableArray<WaitForSnapshot> WaitFors { get; init; }
    ...
    public IEnumerable<(string Name, Value Value, bool IsSensitive)> Properties
    {
        get
        {
            ...
            var healthState = HealthReports.IsEmpty ? (HealthStatus?)null : HealthReports.All(report => report.Status == HealthStatus.Healthy) ? HealthStatus.Healthy : HealthStatus.Unhealthy;
